...
.Scan-Ereignisse an Überwachungsprotokoll senden
BlueXP -Scan-Ereignisse können jetzt an den gesendet werdenxref:task-audit-data-sense-actions.html[Prüfprotokoll]. BlueXP  Classification unterstützt auch das Senden von Klassifikationsereignissen (wenn ein Scan initiiert wird und wenn er endet) an den link:https://docs.netapp.com/us-en/bluexp-setup-admin/task-monitor-cm-operations.html#audit-user-activity-from-the-bluexp-timeline["BlueXP  Zeitplan"^]. Das Senden von Klassifizierungsscan-Ereignissen an die BlueXP -Zeitachse wird nur in Arbeitsumgebungen unterstützt.
