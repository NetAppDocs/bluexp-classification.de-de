...
* **Nur Mapping** Scans bieten nur einen allgemeinen Überblick über Ihre Daten und werden an ausgewählten Datenquellen durchgeführt. Nur-Mapping-Scans benötigen weniger Zeit als Map und klassifizieren Scans, da die nicht auf Dateien zugreifen, um die Daten darin zu sehen.
* **Karten- und Klassify**-Scans ermöglichen ein tiefes Scannen Ihrer Daten.


Mit nur-Mapping-Scans können Sie Ihre Daten schnell scannen und Datenquellen identifizieren, die möglicherweise mehr Forschung erfordern. Anschließend können Sie einen Map & Classify-Scan durchführen.
