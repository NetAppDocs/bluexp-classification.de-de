...
Wenn Ihre Lizenz abläuft oder Sie das BYOL-Limit erreicht haben, wird Data Sense weiterhin ausgeführt, der Zugriff auf die Dashboards wird jedoch gesperrt, sodass Sie keine Informationen zu Ihren gescannten Daten anzeigen können. Nur die Seite _Configuration_ steht zur Verfügung, wenn Sie die Anzahl der eingescannten Volumes reduzieren möchten, um die Kapazitätsnutzung unter das Lizenzlimit zu bringen.
...
NOTE: Wenn das Konto, das Sie verwenden, sowohl eine BYOL-Lizenz als auch ein PAYGO-Abonnement hat, wird Data Sense _beim Ablauf der BYOL-Lizenz nicht_ in das PAYGO-Abonnement verschoben. Sie müssen die BYOL-Lizenz verlängern.
