...
Beachten Sie, dass Sie auch können link:task-quick-start-connector-on-prem.html["Implementieren Sie Data Sense in einer lokalen Website mit Internetzugang"].
...
Derzeit wird Cloud Volumes ONTAP, Azure NetApp Files, FSX für ONTAP, AWS S3 oder Google Drive nicht unterstützt, OneDrive- oder SharePoint Online-Konten, wenn Data Sense auf einer Dark Site bereitgestellt wird.
