...
Wenn Sie noch keinen Connector an Ihrem Offline-Standort installiert haben, https://["Den Stecker einsetzen"^] Jetzt auf einem Linux-Host.
...
Wenn Sie noch keinen BlueXP Connector an Ihrem lokalen Offline-Standort installiert haben, https://["Den Stecker einsetzen"^] Auf einem Linux-Host in Ihrer Offline-Site.
...
* Stellen Sie sicher, dass der Connector über die Berechtigungen zum Implementieren von Ressourcen und zum Erstellen von Sicherheitsgruppen für die BlueXP Klassifizierungsinstanz verfügt. Die neuesten BlueXP-Berechtigungen finden Sie in https://["Die von NetApp bereitgestellten Richtlinien"^].
...
.Schritte
. Laden Sie die BlueXP Klassifizierungssoftware auf einem internetkonfigurierten System von der herunter https://["NetApp Support Website"^]. Die ausgewählte Datei heißt *DataSense-offline-Bundle-<Version>.tar.gz*.
...
* Vergewissern Sie sich, dass Ihre BlueXP Connector-Software auf die neueste verfügbare Version aktualisiert wurde. https://["Siehe die Schritte zur Aktualisierung des Connectors"^].
