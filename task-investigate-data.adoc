...
. Wenn Sie als Berichtstyp CSV ausgewählt haben, wählen Sie **Lokal**, um den Bericht im Browser herunterzuladen. Beachten Sie, dass diese Option den Bericht auf die ersten 10,000 Zeilen beschränkt. Für diese Option sind keine weiteren Optionen erforderlich.
+
Wählen Sie für den vollständigen Bericht **Arbeitsumgebung** aus, und wählen Sie dann aus den entsprechenden Dropdown-Menüs die Optionen **Arbeitsumgebung** und **Volumen**. Geben Sie einen Pfad für den Zielordner** an.
