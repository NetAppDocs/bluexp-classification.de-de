...
....
+
Wenn Sie aktivieren `firewalld` Nach der Installation der BlueXP Klassifizierung müssen Sie Docker neu starten.
...
.. Geben Sie die IP-Adresse oder den Hostnamen der Host-Maschine der BlueXP Klassifizierung ein, damit die Connector-Instanz auf sie zugreifen kann.
.. Geben Sie die IP-Adresse oder den Host-Namen der BlueXP Connector Host Machine ein, damit die BlueXP Klassifizierungsinstanz auf diese zugreifen kann.
