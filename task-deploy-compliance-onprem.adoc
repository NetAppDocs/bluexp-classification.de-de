...
Informationen zu Versionen 1.30 und älteren Versionen finden Sie unter link:task-deploy-multi-host-install-dark-site.html["Installieren Sie die BlueXP Klassifizierung auf mehreren Hosts ohne Internetzugang"].

Das können Sie auch link:task-deploy-compliance-dark-site.html["Installieren Sie die BlueXP Klassifizierung auf einer lokalen Website ohne Internetzugang"] Für vollständig sichere Standorte.


NOTE: Informationen zum Hinzufügen von Scannerknoten für ältere Versionen 1.30 und früher finden Sie unter link:task-deploy-add-scanner-nodes.html["Fügen Sie Scannerknoten zu einer vorhandenen Implementierung hinzu"].
...
[role="quick-margin-para"]
Die ersten 500 TB an Daten, die die BlueXP Klassifizierung in BlueXP scannt, sind 30 Tage lang kostenlos. Um die Daten nach diesem Zeitpunkt weiterhin zu scannen, benötigen Sie ein Abonnement für Ihren Cloud-Provider Marketplace oder eine BYOL-Lizenz von NetApp.
...
On-Premises-ONTAP-Systeme, nicht-NetApp-Dateifreigaben und Datenbankkonten können mit jedem dieser Cloud Connectors gescannt werden.
...
Wie Sie sehen können, gibt es einige Situationen, in denen Sie verwenden müssen https://docs.netapp.com/us-en/bluexp-setup-admin/concept-connectors.html["Mehrere Anschlüsse"].
...
|===
Wenn Sie mehrere BlueXP Klassifizierungs-Hosts nutzen, um eine zusätzliche Rechenleistung zum Scannen Ihrer Datenquellen zu bieten, müssen Sie zusätzliche Ports/Protokolle aktivieren. link:task-deploy-compliance-onprem.html["Siehe zusätzliche Anschlussanforderungen"].
...
Das können Sie auch link:task-licensing-datasense.html["Lizenzierung für die BlueXP Klassifizierung einrichten"] Derzeit. Sie werden auch nach Ablauf der 30-tägigen kostenlosen Testversion nicht belastet.
